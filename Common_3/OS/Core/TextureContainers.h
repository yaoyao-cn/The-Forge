...
static bool loadKTX2TextureDesc(FileStream* pStream, TextureDesc* pOutDesc, void** ppOutData, uint32_t* pOutDataSize)
{
	...
	basist::ktx2_transcoder decoder;

	if (!decoder.init(basisData, (uint32_t)memSize))
	{
		tf_free(basisData);
		LOGF(LogLevel::eERROR, "Failed retrieving ktx2 file information!");
		return false;
	}

	TextureDesc& textureDesc = *pOutDesc;
	textureDesc.mWidth = decoder.get_width();
	textureDesc.mHeight = decoder.get_height();
	textureDesc.mDepth = 1;
	textureDesc.mMipLevels = decoder.get_levels();
	textureDesc.mArraySize = max(1u, decoder.get_layers());
...
#else
	bool isNormalMap = (pOutDesc->mFlags & TEXTURE_CREATION_FLAG_NORMAL_MAP) != 0;
	if (!isNormalMap)
	{
	...
	for (uint32_t s = 0; s < max(1u, decoder.get_layers()); ++s)
	{
		uint32_t w = textureDesc.mWidth;
		uint32_t h = textureDesc.mHeight;
		uint32_t d = textureDesc.mDepth;

		for (uint32_t m = 0; m < decoder.get_levels(); ++m)
		{
			uint32_t rowPitch = 0;
			uint32_t numBytes = 0;
			if (!util_get_surface_info(w, h, textureDesc.mFormat, &numBytes, &rowPitch, NULL))
			...
			if (!decoder.get_image_level_info(level_info, m, s, 0))
			{
				LOGF(LogLevel::eERROR, "Failed retrieving image level information (%u %u)!\n", s, m);
				tf_free(basisData);
				tf_free(startData);
			...
			if (!decoder.transcode_image_level(
				m, s, 0, (void*)data, numBytes,
				basisTextureFormat, 0, rowPitchInBlocks))
			{
				LOGF(LogLevel::eERROR, "Failed transcoding image level (%u %u)!", s, m);
				tf_free(basisData);
				tf_free(startData);
